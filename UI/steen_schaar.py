...
def check_schaar(coords):
    ...
    if afstanden[2] < coef * afstanden[0] and afstanden[3] < coef * afstanden[0] \
        ...
        # cv2.putText(img, "Schaar", (10, 70), cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 0), 3)
        # print('Schaar')
        return True
    else:
        return False
...
def check_steen(coords):
    ...
    if all(cluster):
        # cv2.putText(img, 'Steen', (10, 70), cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 0), 3)
        # print('Steen')
        return True
    else:
        return False
...
def check_move(move):
    result = ''
    if move[0] is True and move[1] is True and move[2] is False and move[3] is False:
        result = 'schaar'
        # print('schaar')
    if all(move) is True:
        result = 'papier'
    elif move[0] is False and move[1] is False and move[2] is False and move[3] is False:
        result = 'steen'
    else:
        result = 'ongeldig'
...
def main():
    ...
    while True:
        ...
        while frame_counter < 40:
            ...
            if results.multi_hand_landmarks:
                for handlms in results.multi_hand_landmarks:
                    ...
                    if schaar:
                        gebaren.append(0)
                    elif schaar is False:
                        steen = check_steen(steen_coords)
                        if steen:
                            gebaren.append(1)
                        elif steen is False:
                            move = check_vingers(coords)
                            antwoord = check_move(move)
                            if antwoord == 'steen':
                                gebaren.append(1)
                            elif antwoord == 'papier':
                                gebaren.append(2)
                            elif antwoord == 'ongeldig':
                                gebaren.append(3)
        ...
        if len(gebaren) == 0:
            return 'rock'
        eind = max(set(gebaren), key=gebaren.count)
        # print(eind)
        if eind == 0:
            cv2.putText(img, 'gebaar is Schaar', (10, 70),
                        cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 0), 3)
        elif eind == 1:
            cv2.putText(img, 'gebaar is steen', (10, 70),
                        cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 0), 3)
        elif eind == 2:
            cv2.putText(img, 'gebaar is papier', (10, 70),
                        cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 0), 3)
        elif eind == 3:
            cv2.putText(img, 'gebaar is ongeldig', (10, 70),
                        cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 0), 3)
        cv2.imshow("image", img)
        # time.sleep(3)
